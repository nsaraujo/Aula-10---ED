...
public class Lista {
    ...
    void addInicio(Object elemento) {
        ...
        if (this.tamanho == 0) {
            Posicao nova = new Posicao(primeira, elemento);
            primeira = nova;
            ultima = primeira;
        } else {
            Posicao nova = new Posicao(this.primeira, elemento);
    ...
    void addFinal(Object elemento) {
        if (tamanho == 0) {
            ...
            Posicao nova = new Posicao(elemento);
    ...
    void addMeio(int pos, Object elemento) {
        if (pos == 0) {
            ...
            if (pos == tamanho) {
                ...
                Posicao anterior = this.pegarCelula(pos - 1);
                Posicao proxima = anterior.getProxima();
                Posicao nova = new Posicao(anterior.getProxima(), elemento);
    ...
    void removerFim() {
        if (!this.posicaoOcupada(0)) {
            ...
            if (tamanho == 1) {
                ...
                Posicao anterior = this.ultima.getAnterior();
        ...
        if (!this.posicaoOcupada(0)) {
            ...
            if (pos == 0) {
                ...
                if (pos == tamanho - 1) {
                    ...
                    Posicao anterior = this.pegarCelula(pos - 1);
                    Posicao atual = anterior.getProxima();
                    Posicao proxima = atual.getProxima();
    ...
    void OrdemDescrecente() {
        if (this.tamanho == 0) {
            System.out.println("[]");
        } else {
            StringBuilder builder = new StringBuilder("[");
            Posicao atual = this.ultima;
    ...
    void numeroPrimo() {
        Posicao atual = primeira;
        for (int i = 0; i < 999; i++) {

            if (atual.getElementInt() > 1) {

                if (nroPrimo(atual.getElementInt())) {
                    System.out.println("Numero primo --> " + atual.getElementInt());

                    Posicao anterior = atual.getAnterior();
